...
def dns_top_clients_per_domain():
    ...
    #VLAN SET
    ib_system.vlanset(config.client_vm,"10.34.220.0")
    #Configure Client#6
...
def dns_top_requested_domain_names():
    logger.info("Preparation:%s",logger.info(sys._getframe().f_code.co_name))
    fp=os.popen("/usr/bin/queryperf -s "+config.grid_vip+" -d ib_data/DNS_Query/DNS_Top_Requested_Domain_Names/QueryPerf_input_TopRequestedDomain.txt")
...
#   enable_recursion_forwarder={"allow_recursive_query":True,"forwarders":[config.client_ip]}
    ...
    fp=os.popen("/usr/bin/queryperf -s "+config.grid_member1_vip+" -d ib_data/DNS_Query/DNS_Top_Timed_Out_Recursive_Queries/queryperf.txt -t 1")
    logger.info("%s",''.join(fp.readlines()))
...
#   fp=os.popen("/usr/bin/queryperf -s "+config.grid_member1_vip+" -d ib_data/DNS_Query/Top_DNS_NXDOMAIN_NOERROR/queryperf.txt")
    fp=os.popen("/usr/bin/queryperf -s "+config.grid_vip+" -d ib_data/DNS_Query/Top_DNS_NXDOMAIN_NOERROR/queryperf.txt")
    logger.info("%s",''.join(fp.readlines()))
...
def rpz_security():
    ...
    response = ib_NIOS.wapi_request('POST', object_type="zone_rp", fields=json.dumps(data1))
    ...
    grid =  ib_NIOS.wapi_request('GET', object_type="grid")
    ref = json.loads(grid)[0]['_ref']
    request_restart = ib_NIOS.wapi_request_('POST', object_type = ref + "?_function=requestrestartservicestatus")
    restart = ib_NIOS.wapi_request_('POST', object_type = ref + "?_function=restartservices")
    logger.info("Wait for 120 Sec.,")
    sleep(600)
...
def dhcp_top_lease_clients():
    ...
    data = {"members":[{"_struct": "dhcpmember", "ipv4addr":config.grid_member1_vip,"name":config.grid_member1_fqdn}], \
    "network": "41.0.0.0/24", "network_view": "network_view_dhcp"}
    response = ib_NIOS.wapi_request('POST', object_type="network", fields=json.dumps(data))

    range_obj = {"start_addr":"41.0.0.1","end_addr":"41.0.0.100","member":{"_struct": "dhcpmember","ipv4addr":config.grid_member1_vip, \
    "name":config.grid_member1_fqdn},"network_view":"network_view_dhcp", \
    ...
    disable_network_41 = ib_NIOS.wapi_request('GET', object_type="network?network~=41.0.0.0/24")
...
def dhcpv4_usage_statistics():
    ...
    cmd10=os.popen("sudo /import/tools/qa/tools/dras/dras -i "+config.grid_member2_vip+" -n 1 -a 99:66:33:88:55:22")
