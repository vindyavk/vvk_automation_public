...
#Add REST API endpoint
	...
	@pytest.mark.run(order=15)
    	def test_015_create_radius_authentication_policy(self):
	...
	@pytest.mark.run(order=16)
    	def test_016_verify_AD_sshlogin(self):
	...
	@pytest.mark.run(order=17)
    	def test_017_verify_radius_gui_login(self):
            print("Testcase 17 started")
            output = os.popen("curl -k -u user1:Infoblox@123 -H 'Content-type: application/json' -X GET https://"+config.grid_vip+"/wapi/"+config.wapi_version+"/grid").read()
            print(output)
	...
	@pytest.mark.run(order=18)
        def test_018_Starting_Logs_for_Template_Execution(self):
        ...
        def test_019_Add_Authoritative_zone(self):
		logging.info("Create Auth Zone")
                ...
                response = ib_NIOS.wapi_request('POST', object_type="zone_auth", fields=json.dumps(data),user="user1",password="Infoblox@123")
                res=json.loads(response)
                logging.info(response)
                read  = re.search(r'201',response)
                for read in  response:
        ...
        def test_020_Stopping_Logs_for_Template_Execution(self):
                logging.info("Starting Outbound Worker Logs to check logs in outbound_user")
