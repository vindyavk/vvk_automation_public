...
#Read all the files from the given path
  ...
  opendir my $dir, "$path" or warn "ERROR: Cannot open directory: $!";
...
if(! -e "Configuration_copy_original.xml") {
...
my $configuration_ref = XMLin("Configuration_copy_original.xml",'KeyAttr' => []);

if(defined($option{w})) {
  if ($$configuration_ref{WHITELIST}{movetowhitelist} =~ m/pcap$/) {
    warn "Sorry, Move to whitelist test case is not supported as yet for pcaps, only works for programs, please specify a program name\n";
  }
  ...
  if(scalar(@move_to_whitelist_cases)==0) {
   warn "Move to whitelist dnst_program is not specified in Configuration.xml.. Aborting\n";
  }
}
...
{eval "require Infoblox";warn if $@;Infoblox->import();}  # use the just-now uploaded Perl API.
...
#Redirect the execution details into a file  
    ...
    open(FH,"> $redirect_file") or warn "Could not write the test case details file.\n";
...
print "KKKKKK";
print $$configuration_ref{SYSTEM_CONFIG}{TUNS_MGMT_IP};
print "KKKKKK";
      pcap_file_copy($$configuration_ref{SYSTEM_CONFIG}{TUNS_MGMT_IP},$program_name);
     #pcap_file_copy($$configuration_ref{SYSTEM_CONFIG}{TUNC_MGMT_IP},$program_name); ## This needs to be uncommented once the vm-12-04d is up, guru commented it to qualify dnsm
...
sub dnst_client_call
{
  ...
  if($program_name =~ m/pcap$/) {
    ...
    if ($out ne '') {`cat fooc.err >> $redirect_file`;warn("Had some problem running pcap $program_name.  The output was\n$out\n"); }
...
#Copy the execution commands to redirect file 
    ...
    $val="";
    my $sshlogin_check="";
    my $exp = Expect->spawn($ssh_command) or warn "Cannot spawn $ssh_command: $!\n";
    $exp->log_stdout(0);
...
sub pcap_file_copy
{
  ...
  my $output = $ssh->capture({stdin_discard => 0, stderr_to_stdout => 0},"cd;ls"); $ssh->error and   warn "remote ls command failed: " . $ssh->error;
  $ssh->system("exit");
  my @values = split('\n', $output);
    ...
    if ($? == 0) {
      ...
      if ($? == 0) {
        print "\nINFO: addkeys was successful.\n";
         ...
         warn " addkeys apparently failed.  If not using the grid's VIP (usually LAN, but maybe MGMT), then you have to do the addkeys yourself first.\n";
      }
    }
    sleep 20;
}
...
sub test_env_details
{
  ...
  if ($cmd_output =~ /CLIENT_VERSION=(.*)$/) {
    ...
    warn "Could not determine NIOS version of $ssh_ip.  Command output was $cmd_output\n";
  }
...
sub update_forwarder
{
  ...
  unless( @result ){
            warn("Get grid DNS failed: ", $session->status_code() . ":" . $session->status_detail());
                   }
  ...
  $session->modify($grid_dns) or warn("Modify grid DNS failed: ", $session->status_code() . ":" . $session->status_detail());
...
sub set_dnst_detection_threshold
{
  ...
  my $exp = Expect->spawn($command) or warn "Cannot spawn $command: $!\n";
...
sub initial_preparation
{
 ...
 unless ($grid) {
     warn("Get Grid failed: ", $session->status_code() . ":" . $session->status_detail());
  }
  $grid->remote_console_access("true");;
  $grid->support_access("true");
  $session->modify($grid) or warn("Modify Grid failed: ", $session->status_code() . ":" . $session->status_detail());
  sleep 5;
...
#Enable DNS
  ...
  unless ($member_dns) {
    warn("get member DNS failed: ", $session->status_code() . ":" . $session->status_detail());
  }
 ...
 $session->modify($member_dns) or warn("DNS Service enable failed: ", $session->status_code() . ":" . $session->status_detail());
...
#Update Grid Analytics properties
  ...
  $session->modify($grid_analytics) or warn("Modify grid Analytics failed: ", $session->status_code() . ":" . $session->status_detail());
  sleep 5;
...
#Start analytics service
  ...
  $session->modify($member) or warn("Enabling Analytics service action is failed: ", $session->status_code() . ":" . $session->status_detail());
  restart_services();
  sleep 15;
}
...
sub install_rpz_license
{
  ...
  my $exp = Expect->spawn($cli_command) or warn "Cannot spawn $cli_command: $!\n";
  my $cliComplete=0;
...
#Read all the rea_dnst test cases from Configuration xml file
  ...
  foreach(@rea_test_cases) { 
    ...
    @output_log="";
    $redirect_file="$output_dir/$_.log";
    open(OF,"> $redirect_file") or warn "Could not write the test case details file.\n";
    my $testcase_start_time=time(); 
    $status="successful";
    $program_name="$_";
    push(@dnst_programs,$program_name); 
...
sub remove_forward_zone
{
  ...
  $session->remove($zone)
     or warn("Remove zone for $zone_name failed: ",
                $session->status_code(). ":" .$session->status_detail());
...
#See if the zone already exists in the database
  ...
  unless($fwd_zone){
        warn("Construct $zone_name zone object failed: ",
                Infoblox::status_code(). ":" .Infoblox::status_detail());
        }

  $session->add($fwd_zone)
     or warn("Add zone for $zone_name failed: ",
                $session->status_code(). ":" .$session->status_detail());
