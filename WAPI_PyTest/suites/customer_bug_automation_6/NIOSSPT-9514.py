...
class Network(unittest.TestCase):
	...
	@pytest.mark.run(order=1)
        def test_001_enable_recursion(self):
        ...
        def test_002_Create_Auth_zone(self):
        ...
        def test_003_Create_RPZ_Zone(self):
		logging.info("Create Local RPZ Zone")
                grid_member=config.grid_fqdn
                data = {"fqdn":"rpz.com","rpz_type": "LOCAL","grid_primary": [{"name":config.grid_fqdn}],"rpz_severity": "MAJOR","rpz_type": "LOCAL","grid_secondaries": [{"enable_preferred_primaries": False,"grid_replicate": False,"lead": False,"name":config.grid_member2_fqdn},{"enable_preferred_primaries": False,"grid_replicate": False,"lead": False,"name":config.grid_member1_fqdn}]}
	...
	@pytest.mark.run(order=4)
        def test_004_delete_mgmt_IPv4_network_conf_from_one_member(self):
                logging.info("Delete mgmt IPv4 conf from one member")
		ref = get_ref_member()
		data = {"node_info":[{"v6_mgmt_network_setting":{"auto_router_config_enabled": False,"cidr_prefix": 64,"enabled": True,"gateway": "2001:550:40a:2500::1","virtual_ip":config.grid_member2_vip6 }}]}
	...
	@pytest.mark.run(order=5)
        def test_005_enable_mgmt_IPv6_DNS_service(self):
	...
	@pytest.mark.run(order=6)
        def test_006_enable_mgmt_IPv4_and_IPv6_DNS_service(self):
	...
	@pytest.mark.run(order=7)
        def test_007_enable_mgmt_IPv4_and_IPv6_DNS_service_on_Master(self):
                logging.info("Enable dns on IPv4 and IPv6  mgmt ")
	...
	@pytest.mark.run(order=8)
        def test_008_change_DNS_notify_to_MGMT_interface_to_member1(self):
        ...
        def test_009_change_DNS_notify_to_MGMT_interface_to_member2(self):
                logging.info("Enable send notifies from the mgmt interface ")
                ref = get_dnsref_member1()
                data = {"dns_notify_transfer_source":"MGMT"}
                response = ib_NIOS.wapi_request('PUT', ref=ref, fields=json.dumps(data),grid_vip=config.grid_vip)
	...
	@pytest.mark.run(order=10)
        def test_010_named_conf_Grid_Master_for_notify_IPv6_IP(self):
                ...
                sys_log_validation = 'ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o UserKnownHostsFile=/dev/null root@'+str(config.grid_vip)+' " cat /infoblox/var/named_conf/named.conf | grep  \'also-notify.*10.36.*2001:550:40a:2500\'"'
                out1 = commands.getoutput(sys_log_validation)
                #import pdb;pdb.set_trace();
                print out1
                logging.info(out1)
                assert re.search(r'also-notify.*10.36.*2001:550:40a:2500',out1)
	...
	@pytest.mark.run(order=11)
        def test_011_named_conf_Grid_Master_for_allow_transfer_IPv6_IP(self):
                logging.info("Validate named conf on Grid Master")
                sys_log_validation = 'ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o UserKnownHostsFile=/dev/null root@'+str(config.grid_vip)+' " cat /infoblox/var/named_conf/named.conf | grep  \'allow-transfer.*10.36.*2001:550:40a:2500\'"'
                out1 = commands.getoutput(sys_log_validation)
                print out1
                logging.info(out1)
                assert re.search(r'allow-transfer.*10.36.*2001:550:40a:2500',out1)
                logging.info("Test Case 11 Execution Completed")


	@pytest.mark.run(order=12)
        def test_012_clenup(self):
