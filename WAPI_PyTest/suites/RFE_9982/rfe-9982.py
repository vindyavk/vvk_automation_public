...
class Network(unittest.TestCase):
    ...
    def test_017_validate_for_zvelo_data(self):
        time.sleep(40)
        logging.info("Validating category Messages Logs")
    ...
    def test_026_Query_all_32_RPZ_Passthru_when_Subscriber_Service_and_Proxy_RPZ_Passthru_are_disabled(self):
        try:
            ...
            for i in range(32):
                dig_cmd = 'ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o UserKnownHostsFile=/dev/null root@'+config.client_ip1+' "dig @'+str(config.grid_vip)+' pass'+str(i)+' +noedns -b '+config.client_ip1+'"'
    ...
    def test_039_Add_Subscriber_Record_with_SSP_set_to_0(self):
        try:
            ...
            child.sendline('infoblox')
            child.expect('Infoblox >')
            child.sendline('set subscriber_secure_data add '+config.client_ip1+' 32 N/A N/A "ACS:Acct-Session-Id=9999732d-34590346;NAS:NAS-PORT=1813;PCP:Parental-Control-Policy=00000000000000000000000000020040;SSP:Subscriber-Secure-Policy=00;PCC:PC-Category-Policy=00000000000000000000000000000001;SUB:Calling-Station-Id=110361221;BWI:BWFlag=1;BL=facebook.com;WL=bbc.com;IPA:IP6=2620:10a:6000:2500:230:48ff:fed5:d928;PXY:Proxy-All=1;PXP:PXY_PRI='+config.primary_proxy_server+';PXS:PXY_SEC='+config.secondary_proxy_server+'"')
    ...
    def test_067_Validate_all_these_5_RPZ_Passthru_returns_Public_IPs_in_response(self):
        logging.info("Validating Sylog Messages Logs")
        for i in range(5):
            LookFor="response.*NOERROR.*pass"+str(i)+".*IN.*A.*"+config.pxy_log+".*"
            print LookFor
            logs=logv(LookFor,"/var/log/syslog",config.grid_vip)
            logging.info(logs)
            print logs
    ...
    def test_085_Add_Subscriber_Record_with_SSP_ff(self):
        ...
        child.sendline('infoblox')
        child.expect('Infoblox >')
        child.sendline('set subscriber_secure_data add '+config.client_ip1+' 32 N/A N/A "ACS:Acct-Session-Id=9999732d-34590346;NAS:NAS-PORT=1813;PCP:Parental-Control-Policy=00000000000000000000000000020040;SSP:Subscriber-Secure-Policy=ff;PCC:PC-Category-Policy=00000000000000000000000000000001;SUB:Calling-Station-Id=110361221;BWI:BWFlag=1;BL=facebook.com;WL=bbc.com;IPA:IP6=2620:10a:6000:2500:230:48ff:fed5:d928;PXY:Proxy-All=1;PXP:PXY_PRI='+config.primary_proxy_server+';PXS:PXY_SEC='+config.secondary_proxy_server+'"')
        child.expect('Record successfully added')
