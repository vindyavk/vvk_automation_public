...
def create_auth_zone():
    ...
    #print config.grid_zone1
    data={"fqdn":config.grid_zone1,"grid_primary":[{"name": config.grid2_fqdn}],"allow_transfer": [{"_struct": "addressac","address": "Any","permission": "ALLOW"}],"allow_query": [{"_struct": "addressac","address": "Any","permission": "ALLOW"}],"allow_update": [{"_struct": "addressac","address": "Any","permission": "ALLOW"}]}
    print data
    response1=ib_NIOS.wapi_request('POST',object_type='zone_auth',fields=json.dumps(data),grid_vip= config.grid2_vip)
...
def check_auth_zone():
    ...
    if response =='[]':
        ...
        time.sleep(20)
        print ("Restarting the grid ")
        data1={"import_from": config.grid2_vip}
