...
class RFE_5514(unittest.TestCase):
    ...
    def test_014_Add_MultiMaster_Zones_For_DDNS_Updates_under_the_configure_DDNS_properties(self):
        ...
        print("ipv4 reverse map zone is",reverse_ref)
	...
	log_print("***************************************************")
	log_print("NIOS-83806 : add Default Primary and Grid zone to preferred multi-domain DDNS list with same primary name server and validate added nameserver shouldn't be updated twice in dhcpd.conf ")
	data = {"ddns_zone_primaries": [{"dns_grid_primary": config.grid_member2_fqdn,"dns_grid_zone": {"_ref": zone_ref},"zone_match": "GRID"},{"dns_grid_primary": config.grid_member2_fqdn,"zone_match": "ANY_GRID"}]}
	...
	restart_services()
        ...
        try:
            ...
            data = 'zone "test1.com." {\n\tprimary '+config.grid_member2_vip+';'
            if data in file_content:
                log_print("The dhcpd.conf file contains nameserver as part of the preferred multi domain ddns list")
                print(data)
                assert True
		print("Validated NIOS-83806 issue and it is working fine")
            else:
                log_print("The dhcpd.conf file does not contains nameserver as part of the preferred multi domain ddns list")
                assert False
        except IOError as e:
	...
	print("\n")
	log_print("*********************************")
	log_print("NIOS-84344 : Add IPv6 reverse-mapping zone with 4 nameservers to the preferred multi-domain ddns list for configuring DDNS properties and verify the error message")
	data1 = {"ddns_dns_view": "default","ddns_zone_primaries": [{"dns_grid_primary":config.grid_member1_fqdn,"dns_grid_zone": {"_ref": ipv6_reverse_ref},"zone_match": "GRID"},{"dns_grid_primary":config.grid_member2_fqdn,"dns_grid_zone": {"_ref": ipv6_reverse_ref},"zone_match": "GRID"},{"dns_grid_primary":config.grid_member3_fqdn,"dns_grid_zone": {"_ref": ipv6_reverse_ref},"zone_match": "GRID"},{"dns_grid_primary":config.grid_member4_fqdn,"dns_grid_zone": {"_ref": ipv6_reverse_ref},"zone_match": "GRID"}]}
	status,response = ib_NIOS.wapi_request('PUT',ref=networkview_ref,fields=json.dumps(data1), grid_vip=config.grid_vip)
	log_print(response)
	data2 = "For each zone, a maximum of three primary DNS servers can be configured for a DNS failover"
        if data2 in response:
                assert True
	...
	print("\n")
	print("*********************************")
	print("Add IPv4 reverse-mapping zone with 4 nameservers to the preferred multi-domain ddns list for configuring DDNS properties and verify the error message")
	data3 = {"ddns_dns_view": "default","ddns_zone_primaries": [{"dns_grid_primary":config.grid_member1_fqdn,"dns_grid_zone": {"_ref": reverse_ref},"zone_match": "GRID"},{"dns_grid_primary":config.grid_member2_fqdn,"dns_grid_zone": {"_ref": reverse_ref},"zone_match": "GRID"},{"dns_grid_primary":config.grid_member3_fqdn,"dns_grid_zone": {"_ref": reverse_ref},"zone_match": "GRID"},{"dns_grid_primary":config.grid_member4_fqdn,"dns_grid_zone": {"_ref": reverse_ref},"zone_match": "GRID"}]}
	status,response = ib_NIOS.wapi_request('PUT',ref=networkview_ref,fields=json.dumps(data1), grid_vip=config.grid_vip)
        log_print(response)
	if data2 in response:
		assert True
		...
		assert False
	log_print("Validated NIOS-84344 issue and it is working fine")
	log_print("***************************************************")
	print("\n")
	...
	#sleep(30)
        ...
        try:
            ...
            assert False
	print("*****************************")
	print(" NIOS-83493 : Verify that IPv6 reverse-mapping zone shouldn't be updated twice in the dhcpdv6.conf file ")
	print("*****************************")
	auth_zone6 = os.popen('grep "ip6.arpa" dhcpdv6.conf -wc').read()
	auth_zone6 = auth_zone6.replace("\n","")
	print(auth_zone6)
	if auth_zone6 == '1':
		assert True
	else:
