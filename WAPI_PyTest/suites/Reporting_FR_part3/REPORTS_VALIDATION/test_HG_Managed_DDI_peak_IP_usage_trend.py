...
# IP Address Usage Report
    ...
    def test_1_managed_ddi_peak_IP_usage(self):
        
        logger.info ("Input Json Data for Managed DDI peak IP usage trend Report validation")
        
        logger.info("Test:"+sys._getframe().f_code.co_name)
        search_str="search index=ib_ipam_summary report=si_ipam_address_usage_trend_per_5days | eval Peak_IPV4=IPV4_COUNT, Peak_IPV6=IPV6_COUNT, Peak_Total=TOTAL_COUNT | streamstats window=5 avg(Peak_IPV4) as Avg_IPv4_Count, avg(Peak_IPV6) as Avg_IPv6_Count, avg(Peak_Total) as Avg_Total_Count | sort _time | eval \"Time\"=strftime(_time, \"%m/%d/%Y:%H:%M:%S\"), \"5-Day Avg IPv4 Count\"=round(Avg_IPv4_Count,0), \"5-Day Avg IPv6 Count\"=round(Avg_IPv6_Count,0), \"5-Day Avg Total Count\"=round(Avg_Total_Count,0) | rename IPV4_COUNT as \"IPv4 Count\", IPV6_COUNT as \"IPv6 Count\", TOTAL_COUNT as \"Total Count\" | table \"Time\", \"IPv4 Count\", \"IPv6 Count\", \"Total Count\", \"5-Day Avg IPv4 Count\", \"5-Day Avg IPv6 Count\", \"5-Day Avg Total Count\""
