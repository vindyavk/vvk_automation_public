...
#Starting DNS service
	...
	def test_010_Copy_Subscriber_Record_radfiles_to_radclient(self):
        	...
        	dig_cmd = 'scp -pr Subscriber_RADIUS_file root@'+str(config.rad_client_ip)+':/root '
	...
	@pytest.mark.run(order=11)
    	def test_011_From_Radclient_Send_Start_Radius_Message(self):
        	...
        	dig_cmd = 'sshpass -p infoblox ssh -o StrictHostKeyChecking=no -o BatchMode=no  root@'+str(config.rad_client_ip)+' "radclient -q -f Subscriber_RADIUS_file/1k.txt -r 1 '+str(config.grid_vip)+' acct testing123"'
        	dig_result = subprocess.check_output(dig_cmd, shell=True)
        	print dig_result
        	assert re.search(r'',str(dig_result))
