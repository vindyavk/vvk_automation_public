...
class NIOSSPT_11064(unittest.TestCase):
    ...
    def test_005_check_traps_in_receiver(self):
        ...
        cmd=('sshpass -p "infoblox" ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@%s' % (config.client_ip))
	cmd = shlex.split(cmd)
	cmd.append('grep -i -c "The NTP service is out of synchronization" trap_data.txt')
	process = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE,stderr=subprocess.PIPE)
	count = process.communicate()
	cmd=('sshpass -p "infoblox" ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@%s' % (config.client_ip))
	cmd = shlex.split(cmd)
	cmd.append('grep -i -c "The NTP service is out of synchronization" trap_data.txt')
	process = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE,stderr=subprocess.PIPE)
	count1 = process.communicate()
	print(count[0],count1[0])
        if int(count[0]) > 0 and int(count1[0]) > 0 :
            assert True
        else:
            assert False
    ...
    def test_clean_up(self):
        ...
        """
        get_ref = ib_NIOS.wapi_request('GET', object_type="member?_return_fields=ntp_setting", grid_vip=config.grid_member1_vip)
        for ref in json.loads(get_ref):
