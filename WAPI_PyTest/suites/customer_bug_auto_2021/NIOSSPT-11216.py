...
class NIOSSPT_11216(unittest.TestCase):
	...
	try:
                  ...
                  child.expect('#')
                  child.sendline('cp /import/qaddi/API_Automation_27_06/WAPI_PyTest/suites/prasad_k/11216bird.conf /import/qaddi/API_Automation_09_09_21/WAPI_PyTest/suites/customer_bug_auto_2021')
                  child.expect('#')
                  child.sendline('chmod 777 /import/qaddi/API_Automation_09_09_21/WAPI_PyTest/suites/customer_bug_auto_2021/11216bird.conf')
                  child.expect('#')
        except:
                  ...
                  child.close()
	with open('/import/qaddi/API_Automation_09_09_21/WAPI_PyTest/suites/customer_bug_auto_2021/11216bird.conf', 'r') as file :
	 	filedata = file.read()
	filedata = filedata.replace('neighbor 10.0.0.2', 'neighbor '+config.grid_vip)
	with open('/import/qaddi/API_Automation_09_09_21/WAPI_PyTest/suites/customer_bug_auto_2021/11216bird.conf', 'w') as file:
	  file.write(filedata)
	print("sleep")
	sleep(10)
	...
	try:
                  ...
                  child.sendline('infoblox')
                  child.expect('#')
                  child.sendline('cp /import/qaddi/API_Automation_09_09_21/WAPI_PyTest/suites/customer_bug_auto_2021/11216bird.conf /usr/local/etc/')
                  #child.expect('password: ')
                  #child.sendline('infoblox')
                  child.expect('#')
        except:
	...
	i = child.expect(['command.','Infoblox >'])
	print("################",i)
	if i == 0:
	...
	print("#####################",data)
	if (data in output1) or (data in output2):
	    assert True
	else:
	    assert False	
	print(data)
	...
	data = ['Neighbor '+config.client_v4,'interface address '+config.client_v4]
	print(data)
	for i in data:
        	if (i in output1) or (i in output2):
                	assert True
			print(i)
        	else:
                	assert False
...
####################################################
    ...
    def test_34_Run_bird_conf_file_to_start_the_neighborship_process(self):
        ...
        try:
                  ...
                  child.expect('#')
                  child.sendline('rm -rf /import/qaddi/API_Automation_09_09_21/WAPI_PyTest/suites/customer_bug_auto_2021/11216bird.conf')
                  child.expect('#')
                  child.sendline('cd /import/qaddi/API_Automation_09_09_21/WAPI_PyTest/suites/customer_bug_auto_2021')
                  child.expect('#')
                  sleep(10)
                  child.sendline('ls')
                  child.expect('#')
