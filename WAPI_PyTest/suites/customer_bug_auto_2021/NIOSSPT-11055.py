...
class NIOSSPT_11055(unittest.TestCase):
	...
	print("Testcase 2 started")
        ...
        user={"name":"user1_radius","password":"user1_radius","admin_groups":["superuser"],"auth_type":"REMOTE"}
        get_ref = ib_NIOS.wapi_request('POST',object_type="adminuser", fields=json.dumps(user))
        if (get_ref[0] == 400):
            print("Duplicate object \'user1_radius\' of type \'adminuser\' already exists in the database")
            assert True
        else:
            print("User \'user1_radius\' has been created")
    ...
    def test_004_verify_radius_sshlogin(self):
        ...
        child = pexpect.spawn('ssh -o StrictHostKeyChecking=no user1_radius@'+config.grid_vip)
    ...
    def test_005_verify_radius_gui_login(self):
    	...
    	get_ref = ib_NIOS.wapi_request('GET',object_type="grid",user="user1_radius",password="infoblox")
    ...
    def test_007_change_auth_type_for_user(self):
        print("Testcase 7 started")
        logging.info("Get user1_radius user reference")
        response = ib_NIOS.wapi_request('GET', object_type="adminuser?name=user1_radius")
        print(response)
	...
	data = {"name":"user1_radius","password":"user1_radius","auth_type":"LOCAL"}
        response = ib_NIOS.wapi_request('PUT',fields=json.dumps(data),ref=auth_ref)
        print("#######################",response)
        logging.info(response)
        read  = re.search(r'200',response)
        for read in response:
    ...
    def test_008_verify_sshlogin_radius_user(self):
    ...
    def test_009_verify_gui_login_radius(self):
        print("Testcase 9 started")
        get_ref = ib_NIOS.wapi_request('GET',object_type="grid",user="user1_radius",password="user1_radius")
        logging.info(get_ref)
        print("#######################",get_ref)
        read  = re.search(r'200',get_ref)
        for read in get_ref:
    ...
    def test_010_cleanup_data(self):
	...
	response = ib_NIOS.wapi_request('DELETE',ref=radiusref)
        ...
        response = ib_NIOS.wapi_request('PUT',fields=json.dumps(data),ref=ref1)
	res2 = ib_NIOS.wapi_request('GET', object_type="adminuser?name=user1_radius")
        print(res2)
