...
class NIOSSPT_10638(unittest.TestCase):
    ...
    def test_000_setup(self):
        ...
        '''Enable DNS over MGMT'''
        if '10.36.' in config.grid_vip:
            display_msg("Enable DNS service over MGMT")
            response = ib_NIOS.wapi_request('PUT', ref=ref['_ref'], fields=json.dumps({"use_mgmt_port":True}))
            display_msg(response)
            if type(response) == tuple:
                display_msg("Failure: Enable DNS Service over MGMT")
                assert False

        '''Check if DNS service is enabled'''
        display_msg("Checking if DNS service is already enabled")
        if json.loads(get_ref)[0]["enable_dns"] == True:
            display_msg("Warning: DNS Service is already enabled")
            return
        else:
            display_msg("Warning: DNS Service is disabled")

            '''"Enable DNS service"'''
            display_msg("Enable DNS service")
            data = {"enable_dns":True}
            response_1 = ib_NIOS.wapi_request('PUT', ref=ref['_ref'], fields=json.dumps(data))
            display_msg(response_1)
            if type(response_1) == tuple:
