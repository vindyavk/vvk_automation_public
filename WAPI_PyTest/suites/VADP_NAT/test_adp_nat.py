...
class adpnat_automation(unittest.TestCase):
    ...
    def test_001_perform_query_for_fixedaddress_without_enabling_nat_in_grid_security_properties(self):
        ...
        dig_cmd = 'dig @'+str(config.grid_member1_vip)+'  authors.bind A -b '+config.config_eth1

        print(dig_cmd)
        out = commands.getoutput(dig_cmd)
        print(out)
        sleep(15)
        sys_log_validation = 'ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o UserKnownHostsFile=/dev/null root@'+str(config.grid_member3_mgmt_vip)+' " tail -n 25 /var/log/syslog | grep -i \"author.*\"" '
        print(sys_log_validation)
        out1 = commands.getoutput(sys_log_validation)
        print(out1)

        print out1
        assert re.search(r'.*src='+config.config_eth1+'.*nat=1.*fqdn=authors.bind hit_count=1',out1)
    ...
    def test_017_test_adp_nat_settings_with_endport_as_65535_and_performing_queries(self):
        ...
        print out1
        assert re.search(r'.*src=10.36.200.11.*nat=1 nfpt=33 nlpt=33 fqdn=authors.bind hit_count=1',out1) 
