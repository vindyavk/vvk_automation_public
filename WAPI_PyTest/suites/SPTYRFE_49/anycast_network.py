...
def parse_get_lab_info(data):
    """
    data     : Hostname/Network/VRID
    Returns  : Dictionary containing lab unit info
    """
    output = os.popen("get_lab_info -H "+data).read()
    if 'ERROR' in output:
        output = os.popen("get_lab_info -S "+data).read()
        if 'ERROR' in output:
            output = os.popen("get_lab_info -R "+data).read()
            if 'ERROR' in output:
                print(output)
                assert False
...
def fetch_network_details(NETWORK):
    info_dict = parse_get_lab_info(NETWORK)
    NETMASK = info_dict["NETMASK"]
    V6NETWORK = info_dict["V6NETWORK"]
    V6CIDR = info_dict["V6CIDR"]
...
def change_the_IPs_of_the_anycast_client_and_check_if_the_client_pings_with_new_IPs(vm_id,ipv4_addr,NETMASK,ipv6_addr,V6CIDR):
    ...
    try:
        ...
        child.sendline("\r")
        login_required = child.expect(["#","login:"])
        if login_required == 1:
            child.sendline("root")
...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        info_dict = parse_get_lab_info(vm_id)
        ipv4_addr = info_dict["LAN_IPADDR"]
        ipv6_addr = info_dict["LAN_IPV6ADDR"]
        ipv6_addr = os.popen('/import/qaddi/sramanathan/IBQACI/ipv6_normalization.pl '+ipv6_addr).read()
        NETWORK = '10.35.0.0'
        NETMASK = info_dict["LAN_NETMASK"]
        V6CIDR = info_dict["LAN_CIDR"]
    ...
    try:
        ...
        f.write('anycast_client_v6=\"'+ipv6_addr+'\"\n')
        f.close()
