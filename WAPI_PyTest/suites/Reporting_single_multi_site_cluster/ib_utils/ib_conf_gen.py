...
def splunkrc(reporting_member1_ip,port='7089',splunk_version='6.3.3'):
    fin=os.popen("ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o UserKnownHostsFile=/dev/null 2>/dev/null root@"+reporting_member1_ip+" iptables -I INPUT -p tcp --dport "+port+" -j ACCEPT")
    fin=os.popen("ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o UserKnownHostsFile=/dev/null 2>/dev/null root@"+reporting_member1_ip+" /root/get_splunk_admin_password -c")
    ...
    conf_var={}
    conf_var["PORT"]=port
    conf_var["HOST_IP"]=reporting_member1_ip
