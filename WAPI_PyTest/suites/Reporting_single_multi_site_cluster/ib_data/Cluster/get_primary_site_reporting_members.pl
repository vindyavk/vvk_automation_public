...
#Retriving Indexers HW ID to perform system related activity. 
 ...
 my @result = $session->get( object=>"Infoblox::Grid::Reporting");
 my $primary_site =  $result[0]->primary_site();
 my @result = $session->search( object=>"Infoblox::Grid::Member",name=>".*",   'extensible_attributes' => { 'ReportingSite' => $primary_site });
 foreach my $i (@result) {
   my $member=$i->name();
   $member =~ m/(\d+)-(\d+)-(\d+)-(\d+)/g;
   my $ip="$1.$2.$3.$4";
