...
def import_csv(csv_file,gm_ip=config.grid_vip):
     rc=subprocess.call(['perl',path+'/ib_utils/ib_import_CSV.pl',gm_ip,csv_file])
     return rc
...
def enable_forwarders_recursion(gm_ip):
    rc=subprocess.call(['perl', path+'/ib_data/Security/enable_recursion_and_forwarders.pl',gm_ip])
    return rc
'''
This Module is to disable the Recursion, Zone transfer and forwarders.
'''
def disable_forwarders_recursion(gm_ip):
    rc=subprocess.call(['perl', path+'/ib_data/Security/disable_forwarder_and_recursion.pl',gm_ip])
    return rc
...
def enable_forwarders_update(gm_ip):
    rc=subprocess.call(['perl', path+'/ib_data/Security/enable_update_zone_transfer.pl',gm_ip])
    return rc

def disable_forwarders_update(gm_ip):
    rc=subprocess.call(['perl', path+'/ib_data/Security/disable_update_zone_transfer.pl',gm_ip])
    return rc
...
def add_rpz_records(gm_ip):
    rc=subprocess.call(['python',path+'/ib_data/Security/rpz_records.py',gm_ip])
    return rc
...
def queries_rpz_records(gm_ip):
    rc=subprocess.call(['sh',path+'/ib_data/Security/dig.sh',gm_ip])
    return rc
...
"""
def download_pm(gm_ip):
    rc=subprocess.call(['getPAPI',gm_ip,path+''])
    return rc
...
def set_time_zone(gm_ip,tz='(UTC) Coordinated Universal Time'):
    rc=subprocess.call(['perl', path+'/ib_data/Grid_properties/enable_Grid_settings.pl',gm_ip,tz])
    return rc
...
def enable_reporting(gm_ip):
    rc=subprocess.call(['perl', path+'/ib_data/Reporting_Properties/reporting.pl',gm_ip])
    print("Enabled Reporting properties")
    return rc
...
def configure_reporting_site(gm_ip,rm_ip,site):
    rc=subprocess.call(['perl', path+'/ib_data/Reporting_Properties/Configure_ReportingSite.pl',gm_ip,rm_ip,site])
    return rc
...
def configure_multi_site(gm_ip,s1,s2):
    rc=subprocess.call(['perl', path+'/ib_data/Reporting_Properties/configure_primary_site.pl',gm_ip,s1,s2])
    return rc
...
def configure_single_site(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Reporting_Properties/configure_single_site.pl',gm_ip])
   return rc
...
def register_dcvm(dcvm_ip,gm_ip,scp_user,passwd):
   rc=subprocess.call(['python',path+'/ib_data/Data_Collector/register_dcvm.py',dcvm_ip,gm_ip,scp_user,passwd])
   return rc

def unregister_dcvm(dcvm_ip,gm_ip,scp_user,passwd):
   rc=subprocess.call(['python',path+'/ib_data/Data_Collector/unregister_dcvm.py',dcvm_ip,gm_ip,scp_user,passwd])
   return rc
...
def add_ip_block_group(group_name, group_ips,gm_ip=config.grid_vip):
   dname=path+"/ib_data/DNS_Query/DNS_Query_Trend_Per_IP_Block_Group/"
...
def check_clustering_mode(gm_ip=config.grid_vip):
    fp=os.popen("perl "+ path+"/ib_data/Reporting_Properties/check_cluster_mode.pl "+gm_ip)
    L=fp.readlines()
    return(L[0].rstrip())
...
def get_search_head(gm_ip=config.grid_vip):
    fp=os.popen("perl "+ path+"/ib_data/Reporting_Properties/get_search_head.pl "+gm_ip)
    L=fp.readlines()
    return(L[0].rstrip())
...
def disable_auto_rules(tp_ip):
   rc=subprocess.call(['expect',path+'/ib_data/Security/disable_auto_rule.exp',tp_ip])
   return rc
...
def disable_tp_system_rules(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/disable_tp_system_rules.pl',gm_ip])
   return rc
...
def add_tcp_rules(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/add_tcp_rules.pl',gm_ip])
   return rc
...
def add_udp_rules(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/add_udp_rules.pl',gm_ip])
   return rc
...
def add_rate_limit_rules(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/add_alert_event.pl',gm_ip])
   return rc
...
def disable_security_category(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/disable_security_cat.pl',gm_ip])
   return rc
...
def enable_security_category(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/enable_security_cat.pl',gm_ip])
   return rc
...
def add_rpz_zone(gm_ip,tp_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/add_rpz_zone.pl',gm_ip,tp_ip])
   return rc

def add_analytics_zone(gm_ip,tp_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/add_analytics_zone.pl',gm_ip,tp_ip])
   return rc
...
"""
def add_rpz_data():
   rc=subprocess.call(['python',path+'/ib_data/Security/rpz_data.py'])
   return rc
...
def add_threate_protection_data(member_ip):
   rc=subprocess.call(['python',path+'/ib_data/Security/tp_data.py',member_ip])
   return rc
...
def modify_analityics_properties(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/modify_analityics_properties.pl',gm_ip])
   return rc
...
def enable_tp_service(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/enable_threate_protection_service.pl',gm_ip])
   return rc
...
def enable_analytics_service(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/enable_analytics_service.pl',gm_ip])
   return rc
...
def enable_discovery_service(gm_ip,discovery_fqdn):
   rc=subprocess.call(['perl',path+'/ib_data/Discovery/enable_discovery_service.pl',gm_ip,discovery_fqdn])
   return rc
...
def enable_forwarder_and_recusion(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/enable_forwarder_and_recursion.pl',gm_ip])
   return rc
...
def add_analityics_data(member_ip):
   rc=subprocess.call(['python',path+'/ib_data/Security/analityics_data.py',member_ip])
   return rc
...
def restart_dns_dhcp_service(gm_ip):
   rc=subprocess.call(['perl',path+'/ib_data/Security/restart_service.pl',gm_ip])
   return rc
