...
class VdcaWapiFlexgridActivation(unittest.TestCase):
    ...
    @pytest.mark.run(order=1)
    def test_1_add_offline_member_in_ib_flex_grid(self):
    ...
    @pytest.mark.run(order=2)
    def test_2_enable_ib_flex_in_preprovisioning_for_offline_member(self):
        get_ref = ib_NIOS.wapi_request('GET', object_type="member")
     ...
     #  print "provisioning['hardware_info']: ", provisioning['hardware_info']
        ...
        print hw_in
        assert provisioning["hardware_info"] == hw_in
    ...
    @pytest.mark.run(order=3)
    def test_3_set_hardware_type_as_ib_flex_for_preprovisioned_offline_member(self):
        print member_vip
        sys_cmd ='reboot_system -H '+str(config.hardware_name)+' -a poweroff'
        out1 = os.system(sys_cmd)
        sleep(180)
        vm_sp = 'vm_specs -H '+str(config.hardware_name)+' -M 22 -C 8'
        vm1 = os.system(vm_sp)
        sleep(180)
        exe_sys_cmd ='reboot_system -H '+str(config.hardware_name)+' -a poweron'
        ...
        try:
            ...
            print "---Selected Yes Option---"
            sleep(180)
            #cmd=commands.getstatusoutput('~/WAPI_PyTest/suites/VDCA_Flex_Grid_Activation_License/show_hw_type.exp ' +str(config.grid_member1_vip))
            cmd=commands.getstatusoutput('/import/qaddi/sramanathan/IBQACI/show_hw_type.exp ' +str(config.grid_member1_vip))
    ...
    def test_4_install_organizational_license(self):
        logging.info('-'*15+"Installing Organizational License"+'-'*15)
        cmd1='/import/qaddi/sramanathan/IBQACI/set_license_name.exp ' +str(config.grid_member1_vip)+ ' "Add Flex Grid Activation license\"'
        rc1=os.system(cmd1)
        logging.info(rc1)
    ...
    def test_5_enable_vdca_service(self):
        logging.info('-'*15+"Enable VDCA Service for Master and Member"+'-'*15)
        logging.info("Enable TP Service")
        response = ib_NIOS.wapi_request('GET', object_type="member:dns?host_name=%s"%config.grid_member1_fqdn)
        ...
        modify_disable_field_1 = ib_NIOS.wapi_request('PUT',object_type=ref_1,fields=json.dumps(data1))
        logging.info(modify_disable_field_1)
        print modify_disable_field_1
        ...
        get_enable_service = ib_NIOS.wapi_request('GET', object_type = ref_1+'?_return_fields=enable_dns_cache_acceleration')
        service_enable=json.loads(get_enable_service)
        logging.info(get_enable_service)
        print get_enable_service
        disable_field = service_enable['enable_dns_cache_acceleration']
        print disable_field
        sleep(20)
        assert service_enable["enable_dns_cache_acceleration"] == True
    ...
    def test_6_perform_set_membership_for_ib_flex_enabled_preprovisioned_offline_member(self):
        
        #set_membership='~/WAPI_PyTest/suites/VDCA_Flex_Grid_Activation_License/set_membership.exp ' +str(config.grid_member1_vip)+'  '+str(config.grid_vip)
        set_membership='/import/qaddi/sramanathan/IBQACI/set_membership.exp  ' +str(config.grid_member1_vip)+'  '+str(config.grid_vip)
    ...
    def test_7_enable_vdca_service(self):
        logging.info('-'*15+"Enable VDCA Service for Master and Member"+'-'*15)
        logging.info("Enable TP Service")
        response = ib_NIOS.wapi_request('GET', object_type="member:dns?host_name=%s"%config.grid_member1_fqdn)
        logging.info(response)
