...
"""
Report : Managed DNS Peak IP Usage Trend
Adding Zones and RR's and performing Dig operation.
Author: Manoj Kumar R G
"""
def managed_dns_peak_usage_trend():
    ...
    cmd = os.system("for i in {1..10};do dig @"+config.grid_vip+" arec.abc.com in a;done")
    print(cmd)

    cmd = os.system("for i in {1..10};do dig @"+config.grid_vip+" aaaa.abc.com in aaaa;done")
    print(cmd)


"""
Report : Managed DDI Peak IP Usage Trend
Create a 10.0.0.0/8 network and range and request leases from both IPv4 and IPv6
Author: Manoj Kumar R G
"""

def managed_ddi_peak_usage_trend():
    # Add ipv4 Network
    ...
    logger.info("Add Range '10.0.0.1 - 10.9.255.255' in '10.0.0.0/8' with Grid master as Member assignment")
...
"""
Report: Detailed RPZ Violations by Subscriber ID
Author: Manoj Kumar RG
"""
...
def discovery():
    ...
    papi.enable_discovery_service(config.grid_vip,config.grid_member1_fqdn)
    sleep(180)
    # Add Network
    network_data = {"members":[{"_struct": "dhcpmember", "ipv4addr": config.grid_member1_vip,"name":config.grid_member1_fqdn}], \
                    "network":"10.40.16.0/24","network_view":"default"}
    response = ib_NIOS.wapi_request('POST', object_type="network", fields=json.dumps(network_data))
    ...
    rc=subprocess.call(['perl','ib_data/Discovery/discovery_preparation.pl',config.grid_vip,config.grid_member1_fqdn])

    # Enable Discovery in Network 10.40.16.0/24
    data = {"enable_discovery":True,"discovery_member":config.grid_member1_fqdn}
