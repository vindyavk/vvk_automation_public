...
grid_member6_vip="[GRID_MEMBER6_VIP]"
grid_member6_fqdn="[GRID_MEMBER6_FQDN]"
...
cisco_ise_server_ip="vm-37-34.ad-36.local"
dns_resolver="10.34.98.36"
host_name="cisco_cert3.ad-36.local"
network="10.34.98"
MS_user="test1"
wapi_user_password="Infoblox1234"
...
def splunkrc(indexer_ip,port='7089',splunk_version='6.3.3'):
   ...
   # fin=os.popen("ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o UserKnownHostsFile=/dev/null 2>/dev/null root@"+indexer_ip+" /infoblox/reporting/bin/get_splunk_admin_password -c")
    fin=os.popen("ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o UserKnownHostsFile=/dev/null 2>/dev/null root@"+indexer_ip+" /root/get_splunk_admin_password -c")
    p=fin.readlines()
    print(p)
    passwd= p[0].rstrip()
    conf_var={}
