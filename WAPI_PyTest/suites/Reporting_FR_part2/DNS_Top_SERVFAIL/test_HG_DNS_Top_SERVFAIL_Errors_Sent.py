"""
 ...
 Description         : DNS Top SERVFAIL Errors Sent. 
...
class DNS_Top_SERVFAIL_Errors_Sent(unittest.TestCase):
    @classmethod
    def setup_class(cls):
        logger.info('-'*15+"START:DNS Top SERVFAIL Errors Sent"+'-'*15)
        logger.info("Preparation for DNS Top SERVFAIL Errors Sent Report")
    ...
    def test_1_DNS_Top_SERVFAIL_Errors_Sent(self):
        logger.info("TestCase:"+sys._getframe().f_code.co_name)
        # search_str="search index=ib_dns_summary report=si_top_servfail_sent_queries | lookup dns_viewkey_displayname_lookup VIEW output display_name | bucket span=1h _time | stats sum(COUNT) as SF_QUERIES by NAME | sort -SF_QUERIES | head 10 | eventstats sum(SF_QUERIES) as COUNT_SUM | eval FAIL_PERCENT=round(SF_QUERIES*100/COUNT_SUM, 1) | eval DNPERCENT=NAME + \"(\"+ FAIL_PERCENT + \"%)\" | rename DNPERCENT as \"Domain Name\", SF_QUERIES as \"Queries\" | fields \"Domain Name\", \"Queries\""
        
        search_str="search index=ib_dns_summary report=si_top_servfail_sent_queries | lookup dns_viewkey_displayname_lookup VIEW output display_name | bucket span=1h _time | stats sum(COUNT) as SFR_QUERIES by NAME | sort -SFR_QUERIES | head 10 | eventstats sum(SFR_QUERIES) as COUNT_SUM | eval DNPERCENT=NAME | rename DNPERCENT as \"Domain Name\" | fields \"Domain Name\""     
    ...
    def teardown_class(cls):
        logger.info('-'*15+"END:DNS Top SERVFAIL Errors Sent"+'-'*15)
        print("DELETE")
