"""
 ...
 Report Name          	: 	1) DNS Traffic Control Resource SNMP Trend (Detailed)
...
TEST Steps:
	...
	3.  Validation : comparing Search results with Retrieved 'DNS Traffic Control Resource SNMP Trend' with delta .5
"""

class dns_traffic_control_resource_snmp_trend(unittest.TestCase):
    @classmethod
    def setup_class(cls):
        logger.info('-'*15+"START : DNS Traffic Control Resource SNMP Trend"+'-'*15)
        
	cls.test1=[
        {"resource":"server2","oid":".1.3.6.1.2.1.2.2.1.1.10","value":'10'}]
        ...
        #time.sleep(1200)
        #time.sleep(350)
    def test_1_dns_traffic_control_resource_SNMP_trend(self):
        logger.info("Test:"+sys._getframe().f_code.co_name)       
        search_str=r"search sourcetype=ib:dns:reserved source=/infoblox/var/reporting/idns_res_snmp.csv index=ib_dtc | bucket span=10m _time | stats avg(value) as value by _time, resource, oid"
    ...
    def teardown_class(cls):
        ...
        lbdn1_get = ib_NIOS.wapi_request('GET', object_type="dtc:lbdn?name=lbdn2")
        ...
        logger.info('-'*15+"END:DNS Traffic Control Resource SNMP Trend"+'-'*15)
