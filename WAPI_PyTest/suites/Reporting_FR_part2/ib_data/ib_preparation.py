...
def dns_top_clients():
    ...
    #bringing 10.34.220.255 interface up
    ib_system.vlanset(config.client_vm,"10.35.0.0")
    print("*********client1**********")
    #client1
    #os.system("sudo ifconfig eth1 inet "+config.client_eth1_ip1+" netmask "+config.client_netmask+" up")
    sleep(60)
    os.system("dig @"+config.grid_vip+" -f ib_data/DNS_Query/DNS_Top_Clients/10k.txt -b "+config.client_eth1_ip1+" > /dev/null 2>&1")
    sleep(5)
    #os.system("sudo ifconfig eth1 inet "+config.client_eth1_ip1+" netmask "+config.client_netmask+" down")
    sleep(60)
    #client2
    #os.system("sudo ifconfig eth1 inet "+config.client_eth1_ip2+" netmask "+config.client_netmask+" up")
    sleep(60)
    os.system("dig @"+config.grid_vip+" -f ib_data/DNS_Query/DNS_Top_Clients/11k.txt -b "+config.client_eth1_ip2+" > /dev/null 2>&1")
    sleep(5)
    #os.system("sudo ifconfig eth1 inet "+config.client_eth1_ip2+" netmask "+config.client_netmask+" down")
    sleep(60)
    #client3
    #os.system("sudo ifconfig eth1 inet "+config.client_eth1_ip3+" netmask "+config.client_netmask+" up")
    sleep(60)
    os.system("dig @"+config.grid_vip+" -f ib_data/DNS_Query/DNS_Top_Clients/12k.txt -b "+config.client_eth1_ip3+" > /dev/null 2>&1")
    sleep(5)
    #os.system("sudo ifconfig eth1 inet "+config.client_eth1_ip3+" netmask "+config.client_netmask+" down")
    sleep(60)
    #client4
    #os.system("sudo ifconfig eth1 inet "+config.client_eth1_ip4+" netmask "+config.client_netmask+" up")
    sleep(60)
    os.system("dig @"+config.grid_vip+" -f ib_data/DNS_Query/DNS_Top_Clients/13k.txt -b "+config.client_eth1_ip4+" > /dev/null 2>&1")
    sleep(5)
    #os.system("sudo ifconfig eth1 inet "+config.client_eth1_ip4+" netmask "+config.client_netmask+" down")
    sleep(60)
    #client5
    #os.system("sudo ifconfig eth1 inet "+config.client_eth1_ip5+" netmask "+config.client_netmask+" up")
    sleep(60)
    os.system("dig @"+config.grid_vip+" -f ib_data/DNS_Query/DNS_Top_Clients/14k.txt -b "+config.client_eth1_ip5+" > /dev/null 2>&1")
    sleep(5)
    #os.system("sudo ifconfig eth1 inet "+config.client_eth1_ip5+" netmask "+config.client_netmask+" down")  
    sleep(60)
...
def dns_top_requested_domain_names():
    logger.info("Preparation:%s",logger.info(sys._getframe().f_code.co_name))
    #fp=os.popen("/usr/bin/queryperf/queryperf -s "+config.grid_vip+" -d ib_data/DNS_Query/DNS_Top_Requested_Domain_Names/QueryPerf_input_TopRequestedDomain.txt")
    fp=os.popen("dduq -i 10.35.132.6 -f ~/API_Automation_08_12_20/WAPI_PyTest/Reporting_FR/ib_data/DNS_Query/DNS_Top_Requested_Domain_Names/QueryPerf_input_TopRequestedDomain.txt")
...
def dns_top_nxdomain_noerror():
    ...
    logger.info("Performing Query using Queryperf")
...
def servfail():
    ...
    """ 
        Create a FORWARD zone in GRID 1
    """
    data = {
    "forward_to": [{
            "address": config.grid_member2_vip,
            "name": config.grid2_fqdn
        }],
    "fqdn": "fwd.com",
    }
    ref_farwarder_zone = ib_NIOS.wapi_request('POST',object_type="zone_forward",fields=json.dumps(data))
    print(ref_farwarder_zone)
    
    """ 
    Enable zone transfer, update and recursion in GRID 1
    """
    grid1_dns_ref = ib_NIOS.wapi_request('GET', object_type='grid:dns')
    #print(config.grid_member2_vip)
    print("---------------------------------------------")
    print(config.grid_vip)
    print(config.grid_member2_vip)
    data = {"forwarders":[config.grid_member2_vip],"forward_only":True,"allow_recursive_query":True,"allow_transfer": [{"_struct": "addressac","address": "Any","permission": "ALLOW"}],"allow_update":[{"_struct": "addressac","address": "Any","permission": "ALLOW"}]}
    
    ref_grid1 = ib_NIOS.wapi_request('PUT', ref=json.loads(grid1_dns_ref)[0]['_ref'], fields=json.dumps(data))
    print(ref_grid1)
    ...
    request_restart = ib_NIOS.wapi_request('POST', object_type = ref + "?_function=requestrestartservicestatus")
    restart = ib_NIOS.wapi_request('POST', object_type = ref + "?_function=restartservices")
    logger.info('Grid1 restart services')
    sleep(10)
 
    """ 
    Create a Authoritative  zone in GRID 2
    """        
    
    data = {"fqdn": "fwd.com","view":"default","grid_primary": [{"name": config.grid_member2_fqdn,"stealth": False}]}
    ref_auth_zone = ib_NIOS.wapi_request('POST',object_type="zone_auth",fields=json.dumps(data), grid_vip=config.grid_member2_vip)
    print(ref_auth_zone)
    
    """ 
    Create A Record in GRID 2
    """ 
    logger.info("Add A record say 'arec.fwd.com/1.1.1.1'")
    a_record = {"name":"arec.fwd.com","ipv4addr":"1.1.1.1"}
    ref_rec = ib_NIOS.wapi_request('POST', object_type="record:a", fields=json.dumps(a_record),grid_vip=config.grid_member2_vip)
    print(ref_rec)
    
    """ 
    Enable zone transfer, update and recursion in GRID 2
    """
    
    grid2_dns_ref = ib_NIOS.wapi_request('GET', object_type='grid:dns',grid_vip=config.grid_member2_vip)
    #print(grid2_dns_ref)
    #data = {"forwarders":[config.grid2_vip],"allow_recursive_query":True,"allow_transfer": [{"_struct": "addressac","address": "Any","permission": "ALLOW"}]}
    ...
    ref_grid2 = ib_NIOS.wapi_request('PUT', ref=json.loads(grid2_dns_ref)[0]['_ref'], fields=json.dumps(data),grid_vip=config.grid_member2_vip)
    print(ref_grid2)
    
    
    """ 
    Enable DNS services in GRID2 
    """
    
    get_ref = ib_NIOS.wapi_request('GET', object_type='member:dns',grid_vip=config.grid_member2_vip)
    ref = json.loads(get_ref)[0]['_ref']
    data = {"enable_dns":True}
    response = ib_NIOS.wapi_request('PUT', ref=ref, fields=json.dumps(data),grid_vip=config.grid_member2_vip)
    print(response)
    
    """
    Disable Zone in GRID 2
    """
    grid2_zone_ref = ib_NIOS.wapi_request('GET', object_type='zone_auth',grid_vip=config.grid_member2_vip)
    for ref in json.loads(grid2_zone_ref):
        if 'fwd.com' in ref['_ref']:
            data = {"disable":True}
            ref_disable_zone = ib_NIOS.wapi_request('PUT', ref=json.loads(grid2_zone_ref)[0]['_ref'],fields=json.dumps(data),grid_vip=config.grid_member2_vip)
            print(ref_disable_zone)
    ...
    logger.info('Grid2 restart services')
    sleep(10)
    '''
    Clear DNS cache 
    '''
    ...
    #sleep(10) 
    output = os.popen("for i in {1..10}; do dig @"+config.grid_vip+" arec.fwd.com a; sleep 2 ; done").read()
    print(output)
    if 'status: SERVFAIL' in  output:
        print("==============================")
        print("found status: SERVFAIL")
        print("==============================")
        assert True
    else:
	assert False 
    ...
    """ 
    Enable Zone in GRID 2
    """
    grid2_zone_ref = ib_NIOS.wapi_request('GET', object_type='zone_auth',grid_vip=config.grid_member2_vip)
    print(grid2_zone_ref)
...
def tunneling():
    logger.info("Enable Auto Rules")
    ref_obj=ib_NIOS.wapi_request('GET', object_type='threatprotection:grid:rule?_return_fields=disabled')
    print(ref_obj)
    data = {"disabled":False}
    for ref in json.loads(ref_obj):
        ref_disable_zone = ib_NIOS.wapi_request('PUT', ref=ref['_ref'],fields=json.dumps(data))
        print(ref_disable_zone)
    ...
    ref = json.loads(grid)[0]['_ref']
    print(ref)
    publish={"member_order":"SEQUENTIALLY"}
    ...
    ref_grid2 = ib_NIOS.wapi_request('PUT', ref=json.loads(grid_dns_ref)[0]['_ref'], fields=json.dumps(data))
    print(ref_grid2)
    
    output = os.popen("dig +retry=0 +timeout=0 @"+config.grid_member1_vip+" any.knjuqljsfyyc2t.com A").read()
    print(output)
    
    output = os.popen("dig +retry=0 +timeout=0 @"+config.grid_member1_vip+" any.knjuqljsfyyc2t.com A +tcp").read()
    print(output)
    output = os.popen("dig +retry=0 +timeout=0 @"+config.grid_member1_vip+" ntsc0yljat A").read()
    print(output)
    output = os.popen("dig +retry=0 +timeout=0 @"+config.grid_member1_vip+" ntsc0yljat A +tcp").read()
    print(output)
    


def traffic_control_resource_availability():

    logger.info("Add Zone dtc1.com")
    sleep(10)
    zone1 = {"fqdn":"dtc1.com","view":"default","grid_primary": [{"name": config.grid_fqdn,"stealth": False}]}
    zone1_response = ib_NIOS.wapi_request('POST', object_type="zone_auth", fields=json.dumps(zone1))			       
    print(zone1_response)
    zone1_get = ib_NIOS.wapi_request('GET', object_type="zone_auth?fqdn=dtc1.com")
    #zone1_ref = json.loads(zone1_get)[0]['_ref']
    for ref in json.loads(zone1_get):
        if 'dtc1.com' in ref['_ref']:
            zone1_ref=ref['_ref']
            
    print(zone1_ref)
    logger.info("Add DTC Server 'server1' with host '10.36.198.2'")
    server1 = {"name":"server1","host":"10.36.198.2"}
    ...
    print(pool1_response)
    pool1_get = ib_NIOS.wapi_request('GET', object_type="dtc:pool?name=pool1")
    pool1_ref = json.loads(pool1_get)[0]['_ref']


  
    logger.info("Add DTC LBDN 'lbdn1' with pool= pool1, zones=dtc1.com, pattern=a1.dtc.com")
    lbdn1 = {"name":"lbdn1","lb_method":"ROUND_ROBIN","pools":[{"pool":pool1_ref}],"types":["A","AAAA","NAPTR"],"patterns":["a1.dtc1.com"],"auth_zones":[zone1_ref]}
    ...
    output = os.popen("for i in {1..2}; do dig @"+config.grid_vip+" arec.fwd.com a; sleep 3 ; done").read()
    print(output)
    

def traffic_control_resource_SNMP_trend():

    '''
    Add SNMP details to grid master
    '''
    ...
    for ref in json.loads(get_ref):
        ...
        if type(response) == tuple:
            if response[0]==400 or response[0]==401 or response[0]==404:
                logger.info("Failure: Enable SNMP in the grid")
                assert False
        else:
            logger.info("Success: Enable SNMP in the grid")
            assert True
    
    
    '''
    Execute snmpwalk -v2c -c public -m all 127.0.0.1 -O n after root to the grid master
    '''
    ...
    try:
        ...
        child.expect('#')
        child.sendline('snmpwalk -v2c -c public -m all 127.0.0.1 -O n &> snmp_walk.txt\n')
        
        child.sendline('egrep -c ".1.3.6.1.2.1.2.2.1.1.10 = INTEGER: 10" snmp_walk.txt')
        child.sendline('\n')
        child.expect('#')
        ...
        "oids": [
            {
                "condition": "EXACT",
                "first": "10",
                "oid": ".1.3.6.1.2.1.2.2.1.1.10",
                "type": "INTEGER"
            }
        ],
        "version": "V2C"
    }
    dtc_snmp_response = ib_NIOS.wapi_request('POST', object_type="dtc:monitor:snmp", fields=json.dumps(data))			       
    print(dtc_snmp_response)
    '''
    Add Zone,Server,pool and lbdn
    '''
    ...
    for ref in json.loads(zone1_get):
        if 'dtc.com' in ref['_ref']:
            zone1_ref = ref['_ref']

    logger.info("Add DTC Server 'server2'")
    server1 = {"name":"server2","host":config.grid_vip}
    server1_response = ib_NIOS.wapi_request('POST', object_type="dtc:server", fields=json.dumps(server1))
    print(server1_response)
    logger.info("Add DTC Pool 'pool2' with servers= server2, monitors=dtc_snmp")
    snmp_monitor_get = ib_NIOS.wapi_request('GET', object_type="dtc:monitor:snmp?name=dtc_snmp")
    snmp_monitor_ref = json.loads(snmp_monitor_get)[0]['_ref']
    
    
    
    server1_get = ib_NIOS.wapi_request('GET', object_type="dtc:server?name=server2")
    server1_ref = json.loads(server1_get)[0]['_ref']

    pool1 = {"name":"pool2","lb_preferred_method":"ROUND_ROBIN","servers":[{"server":server1_ref}],"monitors":[snmp_monitor_ref]}
    pool1_response = ib_NIOS.wapi_request('POST', object_type="dtc:pool", fields=json.dumps(pool1))
    pool1_get = ib_NIOS.wapi_request('GET', object_type="dtc:pool?name=pool2")
    pool1_ref = json.loads(pool1_get)[0]['_ref']
    
    logger.info("Add DTC LBDN 'lbdn2' with pool= pool2, zones=dtc.com, pattern=a1.dtc.com")
    lbdn1 = {"name":"lbdn2","lb_method":"ROUND_ROBIN","pools":[{"pool":pool1_ref}],"types":["A","AAAA","NAPTR"],"patterns":["a1.dtc.com"],"auth_zones":[zone1_ref]}
    lbdn1_response = ib_NIOS.wapi_request('POST', object_type="dtc:lbdn", fields=json.dumps(lbdn1))
    print(lbdn1_response)
