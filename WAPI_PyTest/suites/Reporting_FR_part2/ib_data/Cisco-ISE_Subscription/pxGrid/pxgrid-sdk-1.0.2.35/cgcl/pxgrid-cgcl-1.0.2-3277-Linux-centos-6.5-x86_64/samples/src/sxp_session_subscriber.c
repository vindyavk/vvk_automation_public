...
static void message_callback(jw_dom_node *node, void *arg) {
	UNUSED(arg);			
	jw_dom_node *events_node = jw_dom_get_first_element(node, "events");
	jw_dom_node *event_node = jw_dom_get_first_element(events_node, "event");	
	jw_dom_node *binding_node = jw_dom_get_first_element(event_node, "binding");
	if (binding_node)
	{
		ise_sxpbinding *sxpbinding = NULL;
		ise_sxpbinding_create(&sxpbinding, binding_node);
		ise_sxpbinding_print(sxpbinding);
		jw_dom_node *type_node = jw_dom_get_first_element(event_node, "type");
		if (type_node) 
		{
			ise_print_changetype(type_node);		
		}
...
int main(int argc, char **argv) {
   ...
   const char sess_notif[] = "SXPBindingNotification";
