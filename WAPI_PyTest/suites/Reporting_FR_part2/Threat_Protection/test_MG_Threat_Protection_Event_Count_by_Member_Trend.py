"""
 ...
 ReportName          : Threat Protection Event Count By Member Trend
...
TEST Steps:
      ...
      3.  Validation : comparing Search results with Reterived  'Threat Protection Event Count by Member Trend' report without delta.
"""

class Threat_Protection_Event_Count_by_Member_Trend(unittest.TestCase):
    @classmethod
    def setup_class(cls):
        logger.info('-'*15+"START:Threat Protection Event Count by Member Trend"+'-'*15)
       
        cls.test1=[]
        temp={}
        temp[config.grid_member1_fqdn]="4"
    ...
    def test_1_TP_Event_Count_by_Member_Trend(self):
        logger.info("TestCase:"+sys._getframe().f_code.co_name)
        search_str="search source=ib:ddos:events index=ib_security  (host=\"*\") * * * | noop | noop | bucket span=5m _time | eval SUM_COUNT=ACOUNT+DCOUNT | timechart bins=1000 sum(SUM_COUNT) by host where max in top5 useother=f"
    ...
    def teardown_class(cls):
        logger.info('-'*15+"END:Threat Protection Event Count by Member Trend"+'-'*15) 
