...
#os.system('rm -rf ~/API_Automation/WAPI_PyTest/config.pyc')
...
#os.system('rm -rf ~/API_Automation/WAPI_PyTest/suites/Reporting_FR_part2/config.pyc')
os.system('python -m compileall config.py')
os.system('cp -rf config.py ~/API_Automation/WAPI_PyTest/')
...
#cp -rf /import/qaddi/mgovarthanan/get_splunk_admin_password .
...
if(run_type == 'single_indexer' or  'single_site_cluster' or 'multi_site_cluster'):
    ...
    logger.info("Preparation for DTC availability report (Minute Group Report")
    prep.traffic_control_resource_availability() 

    logger.info("Preparation for DTC SNMP report (Minute Group Report")
    prep.traffic_control_resource_SNMP_trend()
