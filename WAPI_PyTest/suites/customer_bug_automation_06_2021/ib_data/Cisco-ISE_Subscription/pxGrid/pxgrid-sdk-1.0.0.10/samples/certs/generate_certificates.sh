...
function genclientcert {
  ...
  openssl x509 -req -startdate -days 1826 -in $certname.csr -CA $rootname -CAkey $rootkey -set_serial 01 -out $certname.crt
...
function genrootcert {
  ...
  certurl=$2
  openssl genrsa -out $certname.key 1024
  openssl req -new -x509 -days 1826 -key $certname.key -out $certname.crt -subj "/C=US/ST=California/L=San Jose/O=Cisco Systems/OU=SAMPG/CN=$certurl/emailAddress=$certname@cisco.com"
