...
static void query(pxgrid_connection *connection, pxgrid_capability *capability) {
    ...
    if (!jw_dom_context_create(&ctx, &err)
   		|| !jw_dom_element_create(ctx, "{http://www.cisco.com/pxgrid/identity}getSecurityGroupListRequest", &request, &err)
    	)
    {
	...
	if (status != PXGRID_STATUS_OK) {
    		printf("[%s]:[%d]:[%s] ""status=%s\n", __FILE__, __LINE__, __func__, pxgrid_status_get_message(status));
		return;
	}

	jw_dom_node *groups_node = jw_dom_get_first_child(response);
        jw_dom_node *group_node = jw_dom_get_first_child(groups_node);
        while (group_node) {
            ...
            group_node = jw_dom_get_sibling(group_node);
        }
        if(ctx)
...
int main(int argc, char **argv) {
	...
	status = pxgrid_connection_disconnect(connection);
	if (status != PXGRID_STATUS_OK) {
		printf("[%s]:[%d]:[%s] ""status=%s\n", __FILE__, __LINE__, __func__, pxgrid_status_get_message(status));
	}
	printf("*** disconnected\n");
