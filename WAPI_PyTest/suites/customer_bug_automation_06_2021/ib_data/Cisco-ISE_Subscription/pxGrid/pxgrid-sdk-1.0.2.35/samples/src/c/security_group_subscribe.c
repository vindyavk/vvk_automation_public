...
static void message_callback(jw_dom_node *node, void *arg) {
    UNUSED(arg);
    //jw_dom_node *group_node = jw_dom_get_first_child(node);
	jw_dom_node *chgtype_node = jw_dom_get_first_element(node ,"changeType");
	jw_dom_node *group_node = jw_dom_get_first_element(node ,"securityGroup");
	
    if (group_node) {
        ise_security_group *group;
        ise_security_group_create(&group, group_node);
        ise_security_group_print(group);
		if(chgtype_node)  ise_print_changetype( chgtype_node );
        ise_security_group_destroy(group);
    }
}
...
PXGRID_STATUS status;
    ...
    const char cap_name[] = "TrustSecMetaDataCapability";
    ...
    const char sec_gp_chg_notif[] = "securityGroupChangeNotification";
	pxgrid_connection_register_notification_handler(connection, ns_iden, sec_gp_chg_notif, message_callback, NULL);
	sleep(600);
