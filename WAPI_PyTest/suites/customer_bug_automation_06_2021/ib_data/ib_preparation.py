...
def dns_top_requested_domain_names():
    ...
    #os.system("export PATH=$PATH:/home/test2/dduq")
    print("----------------debug line")
    logger.info("---------------debug line")
    fp=os.popen("/home/test2/dduq/dduq -i "+config.grid_vip+" -f "+path+"/ib_data/DNS_Query/DNS_Top_Requested_Domain_Names/QueryPerf_input_TopRequestedDomain.txt")
...
#   enable_recursion_forwarder={"allow_recursive_query":True,"forwarders":[config.client_ip]}
    ...
    fp=os.popen("/home/test2/dduq/dduq -i "+config.grid_member1_vip+" -f "+path+"/ib_data/DNS_Query/DNS_Top_Timed_Out_Recursive_Queries/queryperf.txt -t 1")
    logger.info("%s",''.join(fp.readlines()))
    '''
...
#   fp=os.popen("/usr/bin/queryperf -s "+config.grid_member1_vip+" -d ib_data/DNS_Query/Top_DNS_NXDOMAIN_NOERROR/queryperf.txt")
    ...
    fp=os.popen("/home/test2/dduq/dduq -i "+config.grid_vip+" -f "+path+"/ib_data/DNS_Query/Top_DNS_NXDOMAIN_NOERROR/queryperf.txt")
    logger.info("%s",''.join(fp.readlines()))
    '''
