...
class NIOS_81464(unittest.TestCase):
    ...
    def test_004_try_to_create_a_zone_login_as_local_user(self):
        ...
        #Here i'm changing only user name password remain same
        remove_pyc()
        os.system("sed -i 's/admin/testing/g' config.py")
        sleep(10)
        
        os.system('python -m compileall .')
        sleep(5)
        os.system('cp config.py ~/workspace/NIGHTLY_SNMPV3_PERCY/WAPI_PyTest')
        print(config.username)
        
        print("Add the Authoritative zone test_deny.com in grid through local-user")
        data={"fqdn":"test_deny.com","grid_primary": [{"name": config.grid_fqdn,"stealth": False}]}
        get_res=ib_NIOS.wapi_request('POST', object_type="zone_auth",fields=json.dumps(data))
        print(get_res)
        if type(get_res) == tuple:
            if get_res[0]==200:
                assert False
            elif 'Write permission for zone' in get_res:
    ...
    def test_005_allow_all_interfaces_and_all_show_commands(self):
        print("Change username and password for login as super user")
        remove_pyc()
        os.system("sed -i 's/testing/admin/g' config.py")
        sleep(5)
        os.system('python -m compileall .')
...
"show_version":True},"access_method": ["GUI","API","CLI"]}
    ...
    def test_006_try_to_create_a_zone_after_login_as_local_user(self):   
        ...
        os.system('python -m compileall .')
        #Here i'm changing only user name password remain same
        print("Add the Authoritative zone test_deny.com in grid ")
        data={"fqdn":"test_deny.com","grid_primary": [{"name": config.grid_fqdn,"stealth": False}]}
        get_ref=ib_NIOS.wapi_request('POST', object_type="zone_auth",fields=json.dumps(data))
        print(get_ref)
    ...
    def test_011_try_to_create_a_zone_login_as_local_user_RO(self):  
        ...
        #Here i'm changing only user name password remain same
        remove_pyc()
        os.system("sed -i 's/admin/testing1/g' config.py")
        sleep(5)
        os.system('python -m compileall .')
        print("Add the Authoritative zone test_deny.com in grid through local-user") 
        data={"fqdn":"test_ro.com","grid_primary": [{"name": config.grid_fqdn,"stealth": False}]}
        get_res=ib_NIOS.wapi_request('POST', object_type="zone_auth",fields=json.dumps(data))
        print(get_res)
        if type(get_res) == tuple:
            if get_res[0]==200:
    ...
    def test_012_allow_all_interfaces_and_all_show_commands_RO(self):
        print("Change username and password for login as super user")
        remove_pyc()
        os.system("sed -i 's/testing1/admin/g' config.py")
        sleep(5)
        os.system('python -m compileall .')
