...
###################
		...
		#data = {"grid_secondaries": [{"enable_preferred_primaries": False,"grid_replicate": False,"lead": False,"name": config.grid_fqdn,"preferred_primaries": [],"stealth": False}],"external_primaries": [{"address": config.grid_vip,"name": "grid2","stealth": False,"use_tsig_key_name": False}],"use_external_primary" : True,"fqdn": "zonetransfer.com"}
                data = {"grid_secondaries": [{"enable_preferred_primaries": False,"grid_replicate": False,"lead": False,"name": config.grid2_fqdn,"preferred_primaries": [],"stealth": False}],"external_primaries": [{"address": config.grid_vip,"name": "grid2","stealth": False,"use_tsig_key_name": False}],"use_external_primary" : True,"fqdn": "zonetransfer.com"}
...
#########################
		...
		if data in output:
			...
			assert False
                ...
                print("Restart DHCP Services")
                print("=====================")
                grid =  ib_NIOS.wapi_request('GET', object_type="grid", grid_vip=config.GMC_IP)
                ref = json.loads(grid)[0]['_ref']
                data= {"member_order" : "SIMULTANEOUSLY","restart_option":"FORCE_RESTART","service_option": "ALL"}
                request_restart = ib_NIOS.wapi_request('POST', object_type = ref + "?_function=restartservices",fields=json.dumps(data),grid_vip=config.grid_vip)
                print("Wait for 60 sec.,")
                sleep(60) #wait for 60 secs for the member to get started
		...
		print("================================")
        	...
        	print("\n")
                sleep(20)
                print("grep host record success")
