...
class NIOSSPT_11716(unittest.TestCase):
		...
		data = {"end_addr": "10.0.0.100","member": {"_struct": "dhcpmember","ipv4addr": config.grid_vip,"name": config.grid_fqdn},"network": "10.0.0.0/24","network_view": "default","start_addr": "10.0.0.1"}
		print(data)
...
##NIOS-85099:Able to create an "IPv4 Option Filter" using values exceeding the "length" and "offset" intervals using WAPI and getting the internal error while editing the ###created "IPv4 Option Filter" in GUI 
        ...
        def test_15_Add_IPv4_Option_Filter_with_exceeded_offset_value(self):
                ...
                status,response = ib_NIOS.wapi_request('POST',object_type="filteroption",fields=json.dumps(data),grid_vip=config.grid_vip)
		print("status --> ",status)
                print("response --> ",response)
		#data = "The Offset value must be a value from 0 to 65535"
		data = "Substring offset value  99999999999999999999999999999999999999999999999 is invalid. It should be between 0 and 65535"
		if data in response:
			assert True
			print("\n")
			print(" Validated '"+data+"' in response")
		else:
			assert False
		...
		data = "Substring length value 999999999999999999999999999999999999999999 is invalid. It should be between 1 and 65535"
		if data in response:
			assert True
			print("\n")
			print(" Validated '"+data+"' in response")
...
##############
        ...
        def test_21_Delete_IPv4_Option_Filter_opt_filter1(self):
                print("\n==============================")
                print("Deleting IPv4 Option Filter")
                print("================================")
                option_filter_ref = ib_NIOS.wapi_request('GET',object_type="filteroption?name=opt_filter1",grid_vip=config.grid_vip)
                print(option_filter_ref)
		data = "[]"
                if data in option_filter_ref:
                        assert True
                        print(" The IPv4 option filter was not created with a offset value exceeding ")
                else:
			print(" The IPv4 option filter was created with a offset value exceeding ")
                        assert False
                print("Test Case 21 Execution Completed")

        @pytest.mark.run(order=22)
        def test_22_Delete_IPv4_Option_Filter_opt_filter2(self):
                print("\n==============================")
                print("Deleting IPv4 Option Filter")
                print("================================")
                option_filter_ref = ib_NIOS.wapi_request('GET',object_type="filteroption?name=opt_filter2",grid_vip=config.grid_vip)
                print(option_filter_ref)
                data = "[]"
                if data in option_filter_ref:
                        assert True
                        print(" The IPv4 option filter was not created with a length value exceeding ")
                else:
                        print(" The IPv4 option filter was created with a length value exceeding ")
                        assert False
