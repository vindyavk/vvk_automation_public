...
class DHCPMessageRateTrend(unittest.TestCase):
    ...
    def setup_class(cls):
        ...
        logger.info("Adding 20.0.0.0/8 Network")
        net_obj = {"members":[{"_struct": "dhcpmember", "ipv4addr":config.grid_vip ,"name":config.grid_fqdn}], \
                "network": "20.0.0.0/8", "network_view": "default"}
        network1 = ib_NIOS.wapi_request('POST', object_type="network", fields=json.dumps(net_obj))
        print(network1)
        logger.info("Adding Range 20.0.0.101-20.0.0.110")
        range_obj = {"start_addr":"20.0.0.101","end_addr":"20.0.0.110","member":{"_struct": "dhcpmember","ipv4addr":config.grid_vip,"name": config.grid_fqdn}, \
    ...
    def teardown_class(cls):
    	...
    	delnetwork = ib_NIOS.wapi_request('GET', object_type="network?network=20.0.0.0/8")
