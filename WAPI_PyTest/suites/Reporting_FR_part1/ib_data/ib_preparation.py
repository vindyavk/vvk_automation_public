...
def dns_top_requested_domain_names():
    ...
    #os.system("export PATH=$PATH:~/dduq")
    print("----------------debug line")
    logger.info("---------------debug line")
    #fp=os.popen("~/dduq/dduq -i "+config.grid_vip+" -f "+path+"/ib_data/DNS_Query/DNS_Top_Requested_Domain_Names/QueryPerf_input_TopRequestedDomain.txt")
    fp=os.popen("~/API_Automation/WAPI_PyTest/suites/Reporting_FR_part1/dduq/dduq -i "+config.grid_vip+" -f "+path+"/ib_data/DNS_Query/DNS_Top_Requested_Domain_Names/QueryPerf_input_TopRequestedDomain.txt")
...
#   enable_recursion_forwarder={"allow_recursive_query":True,"forwarders":[config.client_ip]}
    ...
    #fp=os.popen("~/dduq/dduq -i "+config.grid_member1_vip+" -f "+path+"/ib_data/DNS_Query/DNS_Top_Timed_Out_Recursive_Queries/queryperf.txt -t 1")
    ...
    logger.info("%s",''.join(fp.readlines()))
    '''
    fp=os.popen(path+"/ib_data/DNS_Query/DNS_Top_Timed_Out_Recursive_Queries/queryperf.txt")
    falg=0
    for i in fp.readlines():
        print(i)
        flag=flag+1
        if flag==10000:
            ...
            output = os.popen("dig @"+config.grid_member1_vip+" "+i).read()
            print(output)
    '''
...
#   fp=os.popen("/usr/bin/queryperf -s "+config.grid_member1_vip+" -d ib_data/DNS_Query/Top_DNS_NXDOMAIN_NOERROR/queryperf.txt")
    ...
    #fp=os.popen("~/dduq/dduq -i "+config.grid_vip+" -f "+path+"/ib_data/DNS_Query/Top_DNS_NXDOMAIN_NOERROR/queryperf.txt")
...
def dhcp_fingerprint():
   ...
   # Modify DHCP Range 10.0.0.1 to 11.0.0.100 Fingerprint Filter in Range.
    ...
    '''
    # Add Fingerprint Filter to Generate lease for DHCP TOP DEVICE Denied IP Address
    fingerprint_data = {"name":"fingerprint_filter","fingerprint":["Generic Windows OS"]}
    shared = ib_NIOS.wapi_request('POST', object_type="filterfingerprint", fields=json.dumps(fingerprint_data))
    logger.info("Add Fingerprint Filter to Generate lease for DHCP TOP DEVICE Denied IP Address")
    print("&&&&&&&&&&&&&",shared)
   ...
   # Modify DHCP Range 10.0.0.1 to 11.0.0.100 Fingerprint Filter in Range.
    ...
    # Generic Windows OS For DHCP TOP DEVICE Denied IP Address
    logger.info("Generic Windows OS For DHCP TOP DEVICE Denied IP Address")
    #cmd4=os.popen("sudo /import/tools/qa/tools/dras_opt55/dras -i "+config.grid_member1_vip+" -n 1 -w -D -O 55:010304060f")
    cmd4=os.popen("sudo /import/tools/qa/tools/dras_opt55/dras -i "+config.grid_member1_vip+" -n 1 -w -D -O 55:010f03062c2e2f1f2179f92b40d043")
    logger.info("%s", ''.join( cmd4.readlines()))
    sleep(30)
