...
TEST Steps:
	1.  Input/Preparation  : 
		...
		1) server1 - 10.120.22.39 	(Status : Running)
		2) server2 - 10.120.22.193 	(Status : Running)
		3) server3 - 1.1.1.1 		(Status : Error) 
		4) server4 - 1.1.1.2 		(Status : Error)
		5) server5 - 10.120.22.146 	(Status : Running)
...
class dnstrafficcontrolresponsedistributiontrend(unittest.TestCase):
	...
	logger.info("Add DTC Server 'server1' with host '10.120.22.39'")
        server1 = {"name":"server1","host":"10.120.22.39"}
        server1_response = ib_NIOS.wapi_request('POST', object_type="dtc:server", fields=json.dumps(server1))
	
	logger.info("Add DTC Server 'server2' with host '10.120.22.193'")
        server2 = {"name":"server2","host":"10.120.22.193"}
	...
	logger.info("Add DTC Server 'server5' with host '10.120.22.146'")
        server5 = {"name":"server5","host":"10.120.22.146"}
	...
	#fin1=os.popen("~/dduq/dduq -i "+config.grid_vip+" -f "+path+"/ib_data/DNS_Traffic_Control/DNS_Traffic_Control_Response_Distribution_Trend/input.txt")
	fin1=os.popen("~/API_Automation/WAPI_PyTest/suites/Reporting_FR_part1/dduq/dduq -i "+config.grid_vip+" -f "+path+"/ib_data/DNS_Traffic_Control/DNS_Traffic_Control_Response_Distribution_Trend/input.txt")
